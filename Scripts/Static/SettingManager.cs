...
public class SettingManager : MonoBehaviour
{
    ...
    void InitResolution()
    {
        ...
        {
            string resolution = string.Format("{0} x {1}", screen.width, screen.height);
            if(resolutionOptions.Find(x => x.text == resolution) == null)
            {
                resolutionOptions.Add(new Dropdown.OptionData(resolution));
            }
        }
